...
import type { Category, FullStandard, StandardVersion, Topic, TopicEvidenceItem, TopicGroup } from '../types'
...
export async function loadStandardByVersionId(standardVersionId: string): Promise<FullStandard> {
  ...
  const withEvidence = (t: Topic) => ({ ...t, evidence: evidenceByTopic.get(t.id) ?? [] })
...
export function allTopicsFlat(standard: FullStandard) {
  const list: (Topic & { evidence: TopicEvidenceItem[]; categoryCode: string; categoryName: string; groupName?: string })[] = []
